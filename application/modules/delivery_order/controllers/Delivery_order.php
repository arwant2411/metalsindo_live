...
class Delivery_order extends Admin_Controller
{
		...
		public function EditHeader()
    {
		...
		$dtspk = $this->db->query("SELECT * FROM dt_delivery_order_child WHERE id_delivery_order ='$id'")->result();
...
function GetCustomer()
    {
		...
		echo "<div class='col-md-4'><label for='customer'>Customer</label></div>
			...
			<div class='col-md-8' hidden><input type='text' value='$id_customer' class='form-control' id='id_customer' required name='id_customer' readonly >
			<input type='text' value='$nosurat' class='form-control' id='no_surat' required name='no_surat' readonly ></div>";
	}
function AddMaterial_old()
	...
	function GetPenawaran()
    {
		...
		$nama = $nm[0]->name_customer;
		// $dt	= $this->db->query("SELECT a.*, b.nama as nama3, b.hardness as hard FROM dt_spkmarketing as a inner join ms_inventory_category3 as b on a.id_category3 = b.id_category3 WHERE a.id_spkmarketing = '$no_penawaran'  ")->result();
		$dt	= $this->db->query("SELECT a.* FROM dt_spkmarketing as a WHERE a.id_spkmarketing = '$no_penawaran'  ")->result();
		$cr	= $this->db->query("SELECT * FROM tr_inquiry WHERE id_customer = '$id_customer' ")->result();
		$idcr = $cr[0]->no_inquiry;
		...
		<tr id='tabel_penawaran_$loop'>
			...
			<th id='det_lot1_".$loop."'>
			</th>
			<th id='det_lot2_".$loop."'>
			</th>
			<th id='det_lot3_".$loop."'>
			</th>
			<th id='det_lot4_".$loop."'>
			</th>
			<th id='det_lot5_".$loop."'>
			</th>
		</tr>
		";
		}; 
...
function cari_inven1()
	...
	public function SaveNewHeader()
    {
		...
		$numb1++;        
                $stokpakai =  array(
							'id_delivery_order'		=> $code,
							'id_dt_delivery_order'	=> $code.'-'.$numb1,
							'id_material'		    => $dt[idmaterial2],
							'no_alloy'		        => $dt[noalloy2],
							'thickness'		        => $dt[thickness2],
							'width'		        	=> $dt[width2],
							'id_stock'		        => $dt[lot],
							'qty_order'		        => '0',
							'qty_mat'			    => $dt[qty_mat],
							'weight_mat'		    => $dt[weight_mat],
							'lotno'		            => $dt[nolot_mat],
							'lot_slitting'		    => $dt[noslit_mat], 
							'crcl'		            => $dt[crcl2],
							'bentuk'		    	=> $dt[bentuk_mat],
							'remark'		    	=> $dt[remarks],
                            );
                    //$this->db->insert('dt_delivery_order',$stokpakai);
					$this->db->insert('dt_delivery_order_child',$stokpakai);
	...
	public function SaveNewHeaderEdit()
    {
		...
		$numb1++;        
                $stokpakai =  array(
							'id_delivery_order'		=> $id_delivery,
							'id_dt_delivery_order'	=> $id_delivery.'-'.$numb1,
							'id_material'		    => $dt[idmaterial2],
							'no_alloy'		        => $dt[noalloy2],
							'thickness'		        => $dt[thickness2],
							'width'		        	=> $dt[width2],
