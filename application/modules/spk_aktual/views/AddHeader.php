		...
		<form id="data-form" method="post">
			<div class="col-sm-12">
				<div class="row">
										...
										<select id="id_material" name="id_material" class="form-control select" onchange="Caristock()" disabled>
											<option value="">--Pilih--</option>
												<?php foreach ($results['material'] as $material){
													...
													?>
											<option value="<?= $material->id_category3?>" <?=$selx;?>><?= ucfirst(strtolower($material->nama_category2))?>--<?= ucfirst(strtolower($material->nama))?>-<?= ucfirst(strtolower($material->hardness))?>-<?= ucfirst(strtolower($material->thickness))?></option>
												<?php } ?>
										</select>
								</div>
							</div>
					...
					<center>
						<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual/PrintHeader2/'.$id_spkproduksi) ?>" target="_blank"  title="Cetak"><i class="fa fa-print">&nbsp;</i>Cetak Laporan Produksi</i></a>
